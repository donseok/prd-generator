...
`workspace/inputs/projects` 폴더의 모든 파일을 처리하여 통합 PRD 문서를 생성합니다.

## 실행 방법

다음 Python 스크립트를 실행하여 PRD를 생성합니다:
...
- **JSON**: `workspace/outputs/prd/PRD-{timestamp}.json` (TRD/WBS 생성 입력용)

## 처리 파이프라인

1. **Layer 1 (Parsing)**: 각 파일 형식에 맞는 파서로 텍스트 추출
2. **Layer 2 (Normalization)**: Claude를 사용해 요구사항 추출 및 분류 (FR/NFR/CONSTRAINT)
3. **Layer 3 (Validation)**: 완전성/일관성 검증, 신뢰도 낮은 항목 PM 검토 분류
4. **Layer 4 (Generation)**: 최종 PRD 문서 생성

## 주의사항

- Claude API 키가 `.env` 파일에 설정되어 있어야 합니다
- 파일이 많을 경우 처리 시간이 길어질 수 있습니다
- 이미지 파일은 Claude Vision을 통해 분석됩니다
